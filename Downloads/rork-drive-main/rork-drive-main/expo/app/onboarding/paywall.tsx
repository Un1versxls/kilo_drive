...
export default function PaywallScreen() {
  ...
  // Detect if we should enable web/web-based purchase flow
  const isWeb = Platform.OS === "web";
  const canPurchase = isWeb || (offeringsQuery.isSuccess && !!currentPkg);
  ...
  const onStart = () => {
    if (isWeb) {
      // Web purchase flow - start subscription immediately
      startSubscription(planId, cycle);
      upsertAppUser({
        appleUserId: state.profile.appleUserId ?? null,
        email: state.profile.email || null,
    ...
    // Native mobile - use RevenueCat purchase flow
    if (!canPurchase) {
      // If purchases not available, start trial locally and proceed
      startSubscription(planId, cycle);
      if (fromUpgrade) {
        if (router.canGoBack()) router.back();
        else router.replace("/(tabs)/tasks");
        ...
        <View style={styles.footer}>
          {!noOfferings ? (
            <View style={styles.pendingBanner} testID="payments-pending-banner">
              <Text style={styles.pendingTitle}>Subscriptions via App Store</Text>
              <Text style={styles.pendingBody}>
                Apple subscriptions are being configured for this build. Your 3-day trial is active. For direct subscriptions, visit our website.
              </Text>
            </View>
          ) : null}
          <GradientButton
            ...
            variant="gold"
            onPress={onStart}
            disabled={loadingPkgs || (Platform.OS !== "web" && offeringsQuery.isLoading)}
          ...
          <Text style={styles.legal}>
            {loadingPkgs ? (
              ...
              <>
                {planId === "premium" ? (
                  <>
                    {isWeb ? `$${priceFor(plan, "yearly").toFixed(2)} / year` : storePrice ?? fallbackPrice}{" "}
                    {cycle === "yearly" ? "/ yr" : "/ mo"}{canPurchase ? " billed through Apple" : ""}
                  </>
                ) : (
                  <>
                    3 days free, then {storePrice ?? fallbackPrice} {cycle === "yearly" ? "/ yr" : "/ mo"}
                    {canPurchase ? " through Apple" : ""}
                  </>
                )}
              </>
            )}
